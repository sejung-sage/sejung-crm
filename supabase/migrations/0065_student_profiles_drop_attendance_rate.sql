-- ============================================================
-- 0065_student_profiles_drop_attendance_rate.sql
-- 출석률(%) 개념 전체 제거 + absent_count 산식 정정.
-- (원래 0063 이었으나 0062 prefix 충돌 해소로 0064/0065 로 묶어 rename.)
-- ------------------------------------------------------------
-- 운영 결정 (2026-05-20):
--   "출석률"(%) 지표 자체를 운영에서 사용하지 않기로 함.
--   ETL 매핑 갭(0061→0064 fix 반복), 분원별 정책 차이, 운영자별 해석 차이로
--   숫자가 신뢰를 잃어 의사결정에 활용 안 됨. 결석 수·출석 격자·강좌별 출/결
--   raw 카운트는 그대로 유지 — 그 정보만으로도 운영 시야 충분.
--
-- 변경:
--   1) student_profiles.attendance_rate 컬럼 제거.
--   2) absent_count 산식을 attendance_rate (0064) 와 동일 매칭으로 좁힘:
...
COMMENT ON VIEW public.student_profiles IS
  '학생 프로필 (crm_* curated layer). 0065 — attendance_rate(%) 컬럼 제거. absent_count 가 enrollment 매칭 attendance row 만 카운트 (격자와 1:1 일관).';
