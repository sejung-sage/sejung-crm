-- ============================================================
-- 0135_aca_body_pick_deterministic.sql
-- 아카 대표 본문 선택을 결정적으로 — 같은 데이터로 rebuild 해도 결과가 같게.
-- ------------------------------------------------------------
-- 버그 (0133):
--   묶음의 대표 본문을 `(array_agg(b.body ORDER BY length(b.body) DESC NULLS LAST))[1]`
--   로 골랐다. 길이가 같은 본문이 여럿이면 순서가 정해지지 않아, 같은 원본에
--   rebuild_send_analysis_all() 을 두 번 돌리면 서로 다른 본문이 뽑힌다.
--
--   대표 본문에서 '○○○T' 패턴으로 강사를 추출하므로 강사·강사수·금액_배분이
--   실행마다 달라진다. 실측: 9,221 묶음 중 913 묶음(9.9%)이 최장 길이 동률이고,
--   연속 두 번 rebuild 에서 추출된 강사 수가 331 → 332 로 바뀌었다.
--
-- 수정:
--   ORDER BY 에 본문 자체를 2차 정렬키로 추가한다. 길이가 같으면 사전순 첫 본문.
--   어느 쪽을 고르든 강사·강좌 정보는 동일하므로 값의 의미는 달라지지 않고,
--   '실행할 때마다 답이 바뀌는' 성질만 사라진다.
--
-- ROLLBACK: 2차 정렬키 `, b.body` 를 지우고 rebuild 재실행 (비결정성 복귀).
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    -- 길이가 같으면 본문 자체를 2차 정렬키로 써서 대표값을 고정한다 (0135).
    (array_agg(b.body ORDER BY length(b.body) DESC NULLS LAST, b.body))[1] AS body,
    a.sent_on AS d,
    count(*)::int AS r,
