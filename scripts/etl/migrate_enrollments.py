...
매핑 (V_student_class_list → public.enrollments):
  ...
  반고유_코드   → aca_class_id ("{branch_id}-{반고유_코드}", classes 마스터 join 키)
...
def transform(
    ...
    # 강좌 마스터 연결 키. 반고유_코드 NULL 이면 None (FK 없으니 허용).
    class_code = row.get("반고유_코드")
    aca_class_id = f"{branch_id}-{class_code}" if class_code is not None else None
