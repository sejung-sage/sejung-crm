...
const emptyFilters: GroupFilters = { grades: [], schools: [], subjects: [], regions: [], statuses: ["재원생", "수강이력자", "수강 x"], includeStudentIds: [], excludeStudentIds: [], unmappedSchool: false, mappedSchool: false };
...
describe("applyGroupFiltersDev · 복합 필터", () => {
  it("grades=[2] + schools=['휘문고'] + subjects=['수학'] · DC0001 1명만", () => {
    const r = applyGroupFiltersDev(
      DEV_STUDENT_PROFILES,
      { grades: ["고2"], schools: ["휘문고"], subjects: ["수학"], regions: [], statuses: [], includeStudentIds: [], excludeStudentIds: [], unmappedSchool: false, mappedSchool: false },
      "대치",
    );
    expect(r.length).toBe(1);
  ...
  it("grades=[2] + schools=['중동고'] · 대치 중동고 고2 없음 → 0명", () => {
    const r = applyGroupFiltersDev(
      DEV_STUDENT_PROFILES,
      { grades: ["고2"], schools: ["중동고"], subjects: [], regions: [], statuses: [], includeStudentIds: [], excludeStudentIds: [], unmappedSchool: false, mappedSchool: false },
      "대치",
    );
    expect(r.length).toBe(0);
  });
  ...
  it("빈 statuses 는 default '탈퇴 빼고 전체' (3종) → 대치 8명", () => {
    ...
    const r = applyGroupFiltersDev(
      DEV_STUDENT_PROFILES,
      { grades: [], schools: [], subjects: [], regions: [], statuses: [], includeStudentIds: [], excludeStudentIds: [], unmappedSchool: false, mappedSchool: false },
      "대치",
    );
    expect(r.length).toBe(8);
