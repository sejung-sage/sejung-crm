...
describe("GroupFiltersSchema", () => {
  describe("정상 입력", () => {
    it("빈 객체 · 모든 필드 기본값(빈 배열) 적용", () => {
      ...
      expect(r).toEqual({ grades: [], schools: [], subjects: [], regions: [], statuses: [], includeStudentIds: [] });
    });
...
describe("CreateGroupInputSchema", () => {
  describe("정상 입력", () => {
    ...
    it("filters 빈 객체 · 기본 빈 배열로 채워짐", () => {
      ...
      expect(r.filters).toEqual({ grades: [], schools: [], subjects: [], regions: [], statuses: [], includeStudentIds: [] });
    });
  });
